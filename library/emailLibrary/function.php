...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=true) {
  try {
        ...
        $mail->SMTPAuth 	= true;                        				
        $mail->Host 		  = 'srv30.niagahoster.com';  // Masukkan Server SMTP
        $mail->Port 		  = 465;                                      // Masukkan Port SMTP
        $mail->SMTPSecure = 'ssl';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
        $mail->Username 	= 'no-reply@belajarmlm.com';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
        $mail->Password 	= 'belajarmlm77';        						          // Masukkan Password dari Email tsb
        $default_email_from       = 'no-reply@belajarmlm.com';        // Masukkan default from pada email
        $default_email_from_name  = 'BELAJARMLM.COM';           // Masukkan default nama dari from pada email
